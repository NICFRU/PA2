...
class BKPF(server.App):
    ...
    def datahead(self,params):
        df = of.Data(BKPF.link)
        return df.head(10)

    def description(self, params):
        value = params["values"]
        df = of.overview(BKPF.link,True)
    ...
    def unique(self, params):
        ...
        df_string = of.data_str(BKPF.link)
        ...
        data = {"langs" : langs,
                "unique"   : [ int(x) for x in df[value].to_list() ]
                }
        ...
        hover.tooltips = """
        <div>
            <div><strong>Element:  </strong>@langs</div>
